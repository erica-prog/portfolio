...
<img align="left" width="300" height="200" src="assets/architectural%20diagram.png"> **[AI Discourse at Scale: Topic, Sentiment, and Community Dynamics Across Reddit](https://github.com/erica-prog/data-engineering-AI-trends-public.git)**

This collaborative project analyzed 4.2B Reddit comments/submissions (June 2023–July 2024) using Apache Spark distributed processing to quantify public response to generative AI's breakout year, building a scalable ETL pipeline that filtered 445 GB parquet archives down to AI-focused subreddit clusters and applied Spark NLP for sentiment analysis, topic modeling, and temporal aggregation across the 14-month window. Then, partitioned processing across subreddit + month dimensions to generate summary statistics at community and temporal levels, tracking engagement metrics (comment volume, unique users, activity spikes) that surfaced, which communities drove discourse shifts. This project delivered a reproducible snapshot of digital conversation at a technology inflection point—demonstrating distributed data engineering for behavioral analytics at scale with a pipeline that handles billion-row datasets efficiently, enabling real-time insights into community-driven tech adoption patterns using PySpark, Spark NLP, parquet I/O, and distributed aggregations, with deliverables including an interactive website.
...
- ### Simple Option Pricing Model (AAPL Stock)
    - The [European option pricing model](https://github.com/erica-prog/Option-Pricing-/tree/main) uses Black-Scholes closed-form solutions to value AAPL call/put options, processing 67 contracts across multiple expiration dates and calculating theoretical prices, Greeks, and in-the-money probabilities using Python. Then, it was identified that there was a 53% pricing gap between the theoretical value ($2.83) and the market price ($5.97). Volatility smile patterns (0.30-0.60 IV range) were analyzed, and trading volume concentration at $300+ strikes was discovered, indicating bullish sentiment through time-series visualization.
